...
scheafer_400 = cifti_read([template_dir 'Schaefer2018_200Parcels_17Networks_order.dscalar.nii']);

rest_data = '/root_dir/Public_Data/HCPD/processed_data/rsfMRI/xcp_abcd_rest_no_MSM/';
all_subject = dir([rest_data 'sub-*']);
all_subject = {all_subject(:).name}';

out_dir = '/root_dir/wuguowei/code/FC_ontology/FC_net_HCPD_200';
...
for sub = 1:numel(all_subject)
    subname = all_subject{sub};
    sub_dir = [rest_data subname filesep];
    rest1 = dir([sub_dir 'func/*-REST1*_smooth_den-91k_bold.dtseries.nii']);
    rest2 = dir([sub_dir 'func/*-REST2*_smooth_den-91k_bold.dtseries.nii']);
